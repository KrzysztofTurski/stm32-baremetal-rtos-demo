...
{
    /* Stack pre-fill for debug + initial exception frame:
       -16 words reserved for HW-saved context (R0-R3,R12,LR,PC,xPSR) + space */
                           ...
                           void (*TaskLogger)(void))
{
    ...
    osKernelStackInit(1);
    TCB_STACK[1][STACKSIZE - 2] = (int32_t)TaskSonar;

    osKernelStackInit(2);
    TCB_STACK[2][STACKSIZE - 2] = (int32_t)TaskLogger;
...
{
    /* 1ms tick: updates sleep counters and pends PendSV for preemption */
    for (int i = 0; i < NUM_OF_THREADS; i++) {
        if (tcbs[i].sleepTime > 0) {
            tcbs[i].sleepTime--;
            if (tcbs[i].sleepTime == 0 && tcbs[i].state == SLEEPING) {
                tcbs[i].state = READY;
            }
        }
    }
...
void osThreadBlock(threadState_t reason)
{
    /* Called with IRQs disabled by the caller.
       This must be atomic (no lost-wakeup): set state, pend PendSV, then re-enable IRQs. */
